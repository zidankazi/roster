...
**Phase 2 — structured fields + statusline telemetry (mostly agent-safe). Statusline parsing and telemetry precedence implemented and fixture-tested; hook-sourced fields and live wiring pending.**
Grow `StateReading`, parse hook + statusline payloads into it. The parsing and the multi-source precedence logic are **agent-safe** — fixture the JSON payloads exactly as detection is fixtured from grids today ([02](02-state-detection.md)), and the tests are the contract. Implemented so far: `statusline::parse` (payload JSON → the shared `Telemetry`, fixture-tested), and readings that carry it — `StateReading.telemetry`, attached by the `PaneTracker` with the aging rule under "Open questions" below. Hook-sourced fields (`tool`, `permission_mode`, …) are not grown yet; carrying a reading's telemetry into the session model and sidebar is the remaining **agent-safe** leg (`Session::set_reading` currently takes only state and reason, so telemetry stops at the tracker); registering the statusline command and wiring payloads to live panes (`PaneTracker::set_telemetry`) is **keyboard**. (No transcript parsing — see the warning above.)

**Phase 3 — the attention inbox (keyboard, taste).**
The ranked cross-workspace queue in `roster-tui`, verbatim reasons, jump-to-answer. This is a UI-taste milestone; it needs eyes on real fleets. The ordering primitive already exists: `roster_core::attention::rank` (blocked first — destructive asks above plain ones, then longest wait — then working, done, idle). Note it deliberately differs from the sidebar's per-window order in `roster-tui/src/sidebar.rs::state_rank`, which floats done above working; the inbox ranks demands, the sidebar summarizes panes — reconcile consciously if the two ever merge.
