...
- **Multi-source precedence**: statusline refresh cadence vs. instantaneous hook events — define which wins for a field. (Phase 1 settled the state-signal case: hook wins on freshness, screen wins on settled reality. Phase 2 settled telemetry aging, split by how a reading goes wrong: the freshest statusline payload rides every committed reading — never debounced, the scrape has no telemetry to offer — and its fast-moving fields (model, context %, cost) drop back to `None` after `TELEMETRY_STALE_AFTER`, 30s in track.rs, with no newer payload. Rate-limit windows with a reported reset outlive that window on purpose — Claude Code only re-runs the statusline while the conversation moves, so a fleet of idle/blocked panes goes quiet for far longer than 30s while account usage cannot move; the tracker re-derives the reset countdown from the payload's arrival stamp every tick (`PaneTracker::asserted_telemetry` / `RateLimit::aged`) and drops the window the moment its reset passes, since the reset zeroes the used share the reading holds. A window without a reported reset has no horizon to age against and keeps the 30s rule. Per-field arbitration only arises when a second source for a field exists — today statusline is the sole writer.)
