...
roster's sidebar card titles already consume the terminal title Claude Code broadcasts (README: "Claude Code sets it to what it's working on"; see the `app` title handling — a card's first line prefers the live task over the config name). The scrape is pane-agnostic: **every** pane's title is read, not just an identified agent's, so a shell whose prompt sets a title labels itself too and one whose prompt doesn't falls back to the command's basename. Titling is a property of owning a PTY, not of being an agent — gating it on identification was an accident of where the code was first written. Note the honest caveat: **Claude Code setting the title is undocumented** — the official docs describe statusline OSC 8 hyperlinks but not title-setting. So it works empirically today but is not a contract. Keep using it for row labels as a *nice-to-have*, and never let a core signal depend on it. The documented fallback is **implemented**: the statusline payload's `session_name` (Claude Code's own summary of the session) is parsed beside the telemetry (`statusline::Payload` — deliberately *not* a `Telemetry` field, so the name can't age out with the numbers) and stored on the pane keyed to the payload's `session_id` (`Session::set_session_name`: sticky within a session — a nameless payload never strips a known name — but reset when a different session starts reporting, so a `/clear`'d pane can't keep wearing the old task's label). `SidebarEntry::display_name` resolves title → session name → config name. A session whose first prompt is a slash command never gets a title broadcast; before this fallback its card read `claude-code` forever. Known gap: the name is client-model state — neither snapshotted nor replayed like the title's OSC bytes — so after a reattach the card falls back to the config name until the feed's next payload, which for an idle pane may not come.
...
**Phase 1 — the hook bridge, one event end-to-end. Implemented and fixture-tested; payload contract live-verified against Claude Code 2.1.207.**
Implemented as: roster spawns every pane with `ROSTER_PANE` (its pane id) and `ROSTER_HOOK_SOCK` in the environment; Claude Code hooks inherit that environment (checked empirically against the installed Claude Code — the docs don't promise it), so a `roster _hook` command registered for `PermissionRequest`/`PreToolUse`/`Stop` (via `roster --print-hooks`, merged into `~/.claude/settings.json`; the snippet embeds this binary's absolute path) reports the exact ask the instant it fires. `PermissionRequest` pins the pane 🔴 blocked with the verbatim tool + input (`Bash: cargo test`). Clears are **tool-matched**: an approved tool's `PreToolUse` clears only its own ask (a parallel auto-approved tool can't erase a pending one), `Stop` clears anything (end of turn), and subagent events never clear. `PreToolUse` does double duty — the same verbatim tool + input it clears with also becomes the **working card's reason**, replacing the scraped spinner (`✱ Deliberating…` → `Bash: cargo test`): richer than the spinner, and the glyph already says "working", so the reason line is spent on *what* the agent is doing. That activity is shown only while the screen agrees the pane is working (the screen owns the state, the hook owns the wording), refreshes on each `PreToolUse`, and retires on `Stop` or once a settled non-working scrape drops it — so a finished turn can't freeze a stale tool call. Three `roster-proto` frames carry it: `HookBlocked`/`HookClear`/`HookActivity` (tag 15) — `HookActivity` mirrors `HookBlocked`'s shape but is fire-and-forget with no reply, and both the tool (for the clear) and the reason ride it. In plain `roster` the app owns a per-process socket at `/tmp/roster-<uid>/hook/<pid>.sock` — a subdirectory, deliberately outside the session-socket namespace `ls`/`kill`/`attach` probe; in `-s` sessions agents report to the session socket and the server relays to the attached client — detection lives client-side either way, and pins apply only to identified claude panes. The pin outranks the scrape on freshness, **not forever**: the scrape keeps running underneath, and once a pin is past a 2s paint grace, a committed non-blocked scrape drops it — a missed clear (interrupt at the prompt fires no hook; a denial fires none until `Stop`) self-heals within ~1s instead of pinning a wrong 🔴 indefinitely. The bridge never fails a Claude session (silent no-op outside roster, always exits 0, stdin read is deadlined). End-to-end plumbing tests: `hook_bridge_pins_blocked_and_clears_end_to_end` and `hook_activity_replaces_the_working_spinner_reason_and_clears_on_stop` in `crates/roster/tests/smoke.rs` — they fake the hook payloads, so they prove roster's side only. **The payload contract is live-verified against Claude Code 2.1.207** (2026-07-13): in a real claude pane inside roster, with hooks registered exactly as `--print-hooks` emits them, a permission-requiring Bash tool call pinned the card 🔴 with the verbatim tool + input (`Bash: touch /tmp/…`) the moment the dialog rendered, and the attention ranking put it at the top of the sidebar.
...
Grow `StateReading`, parse hook + statusline payloads into it. The parsing and the multi-source precedence logic are **agent-safe** — fixture the JSON payloads exactly as detection is fixtured from grids today ([02](02-state-detection.md)), and the tests are the contract. Implemented so far: `statusline::parse` (payload JSON → the shared `Telemetry`, fixture-tested), readings that carry it — `StateReading.telemetry`, attached by the `PaneTracker` with the aging rule under "Open questions" below — and the **bridge itself**: `roster --print-statusline` prints the `statusLine` snippet to merge into `~/.claude/settings.json` (absolute binary path, manual merge exactly like `--print-hooks`, warning on stderr when a statusline is already configured — see the resolved slot-conflict question below), and `roster _statusline` forwards the stdin session JSON **verbatim** as a `Statusline` frame (`roster-proto` tag 14, fire-and-forget — unlike an ask, it never gets a reply) over the same hook socket, under `_hook`'s exact safety contract (silent no-op outside roster, always exits 0, stdin read is deadlined, prints nothing — stdout would become the pane's visible statusline). Parsing stays client-side in `roster-detect`, the one pinned contract point; in `-s` sessions the server relays the frame verbatim like hook clears. The app routes it to the pane — identified, live agent panes only — into `PaneTracker::set_telemetry`, and every detection tick carries the reading's telemetry into the session model (`Session::set_telemetry`, with `None` clearing on staleness) and onto the card: the **focused** entry with telemetry grows a third line of badges (`telemetry_line` — context % in the severity color, the most-used rate-limit window, then cost; the model badge is dropped, redundant in a Claude-only fleet, and cost trails so it is the first to truncate on a narrow card, keeping context and the limit visible at the default width); an unfocused entry grows the row only when its context alert escalates, and then it shows just that badge (`telemetry_row_visible` is the single gate) — at a glance, context-about-to-compact is the one telemetry reading with attention value, with the accepted trade-off that a healthy-context card's cost and rate-limit readings are visible only while focused. A bridge-less card keeps its exact two-line shape. End-to-end proof: `statusline_telemetry_reaches_the_sidebar_card` in `crates/roster/tests/smoke.rs` — a fake claude pipes the payload through `roster _statusline` and the badge line appears on the real sidebar. **The payload contract is live-verified against Claude Code 2.1.202**: a real claude pane inside roster produced the badge line end-to-end, and the captured payloads (now the fixtures in `roster-detect/tests/fixtures/statusline/`) match every documented field — percentages arrive as integers, and the payload carries additive fields the parser deliberately ignores (`prompt_id`, `effort.level`, `thinking.enabled`, `context_window.current_usage.*`, `workspace.repo.*`). Hook-sourced fields (`tool`, `permission_mode`, …) are not grown yet. (No transcript parsing — see the warning above.)

On top of the per-pane feed sits the **fleet rate-limit layer** — rate limits are account-scoped, one budget shared by every pane, so per-card treatment understates them. `roster_core::fleet_rate_limit` merges the panes' tracker-stamped readings (freshest reading wins per window, windows resolved independently, staleness inherited from the tracker's aging so the live merge can never assert what the cards no longer do), `roster_core::carry_rate_limit` carries the previous fleet reading across quiet feeds — Claude Code re-runs the statusline only while the conversation moves, so an idle or blocked fleet goes quiet far longer than the 30s tracker ageout, while account usage from those panes cannot move meanwhile; a live reading always outranks the carried one (even a zero-countdown window a feed still asserts), a carried window's countdown is re-aged every detect tick by the larger of the monotonic and wall clocks (wall keeps ticking through a system sleep) and retires when its reset passes — trailing reality by at most the 30s tracker freshness, since live readings display as they arrived — a carried window without a reported reset does not outlive the feeds, the carry clears when the last identified agent pane exits, and the carried used share is explicitly a floor (consumption outside this roster instance can raise the real share without moving the display until a feed wakes) — and the sidebar pins the result as a footer: each reported window a labeled usage bar (`5h`, `wk`) with its used share and reset time, quiet below 70% used, the working yellow from 70%, the blocked red from 90% (`rate_limit_alert` owns the thresholds; the footer yields whole on sidebars too short for it, and renders only while an identified agent pane exists and a reading is live or carried — bridge-less sidebars are cell-for-cell unchanged). `roster_core::LimitNotifier` turns the same aggregated stream into edge-triggered toasts — once per threshold per window (warn at 70, the loud red at 90), re-armed when usage falls back below the threshold, which is also how a window reset reads — and the binary wires notices to toast cards. End-to-end proof: `statusline_rate_limits_reach_the_sidebar_footer_and_toast` in `crates/roster/tests/smoke.rs`.

**Phase 3 — the attention inbox (keyboard, taste).**
The ranked cross-workspace queue in `roster-tui`, verbatim reasons, jump-to-answer. This is a UI-taste milestone; it needs eyes on real fleets. The ordering primitive already exists and the sidebar orders by it today: `roster_core::attention` — blocked first (destructive asks above plain ones, then longest wait), then done, then idle, with working at the bottom, because a working agent needs nothing from the human. Within a tier the duration reads in whichever direction that tier's judgment wants: blocked ranks by neglect, so the ask ignored longest leads and cannot be pushed down by a fresh one; every other tier reads as a feed of what just happened, so the most recent arrival leads. An age the model never recorded trails its tier — a pane that has never changed state has earned neither end. The sidebar is that ranking, at exactly one scope: one flat list ranked globally across all workspaces, each card tagged `⧉N` with its home — the cross-workspace ranked view, in-sidebar. (A per-workspace grouping with a `by space · by need` switcher shipped and was deliberately removed: the global ranking is the product's one triage judgment, and offering a second scope diluted it.) The remaining inbox work is the dedicated surface (verbatim reasons, jump-to-answer) on top of the same ranking: one triage judgment, owned in one place, so the sidebar and the inbox can never disagree about who needs you first.

**Phase 4 — answer-from-sidebar, push, telemetry (keyboard). Decision channel + per-pane auto-approve implemented; approve envelope live-verified against Claude Code 2.1.207.**
The **decision channel is wired**: `roster _hook` now waits, deadlined, for a reply on the hook connection, and whoever owns the socket answers with a `HookDecision` frame — the local app from an `Arc<Mutex<HashSet>>` of auto-approved panes (the listener replies inline, before forwarding, and poison-tolerantly), the session server from its own set (told by the client via `SetAutoApprove`, replied *before* the client relay so a slow client can't stall it). The first consumer is a **per-pane auto-approve toggle**, surfaced as a clickable `auto` pill on the card's detail row (an armed pill is always drawn, accent-filled; an unarmed one is revealed on the hovered, selected, or focused card, and a click landing on a *hidden* pill's columns falls through to the card's jump — an invisible control must never arm approval; keyboard: `ctrl-b j`, then `a`): roster answers that pane's `PermissionRequest`s with `allow` while staying observable — the card keeps its real scraped state plus the lit chip, and an auto-approved ask is deliberately *not* pinned 🔴 (the paint-grace would otherwise hold a false blocked against a prompt that never renders). Two semantics are load-bearing and intentional: the toggle is **forward-looking** (it cannot retro-answer an in-flight prompt — only the pane's next asks), and because `HookBlocked` carries no `agent_id` it **necessarily approves the pane's subagent asks too** (pane-level trust). Still ahead on this channel: interactive per-ask allow/deny buttons, push notifications, per-card/session telemetry. **The approve envelope is live-verified against Claude Code 2.1.207** (2026-07-13, the same session that verified Phase 1's contract): with the pane's `auto` pill armed, a permission-requiring Bash tool call ran with no dialog rendered — roster's `allow` reply was honored — while the identical call with the pill disarmed rendered Claude Code's own prompt and pinned the card as in Phase 1. Each is additive on the Phase 1–2 bridge.
...
- **Version pinning**: which minimum Claude Code version to target (e.g. `prompt_id` needs ≥ 2.1.196), and how the bridge degrades on older/newer ones. The statusline payload has been observed live on 2.1.202; all fields the parser maps were present there. The hook payload contract (`PermissionRequest` pin with verbatim tool + input) and the approve envelope have been observed live on 2.1.207.
- **Bridge-install UX**: opt-in flow, namespaced merge into `~/.claude/settings.json` (`hooks` + `statusLine`), and clean uninstall.
- **Mixed-account fleets**: the fleet rate-limit view assumes every pane reports one account — the statusline payload carries no account identity, so panes running under different accounts (e.g. distinct `CLAUDE_CONFIG_DIR`s) would alternate the freshest-wins reading between accounts, flapping the footer and re-firing the crossing toasts on every flip. Accepted for now (one account per roster session is the overwhelmingly common shape); grouping needs an account field in the payload before it can be built honestly.
- **Multi-source precedence**: statusline refresh cadence vs. instantaneous hook events — define which wins for a field. (Phase 1 settled the state-signal case: hook wins on freshness, screen wins on settled reality. Phase 2 settled telemetry aging: the freshest statusline payload rides every committed reading — never debounced, the scrape has no telemetry to offer — and drops back to `None` after `TELEMETRY_STALE_AFTER`, 30s in track.rs, with no newer payload; that per-pane rule is deliberately uniform, and the account-scoped survival of rate limits across quiet feeds lives one layer up, in the fleet carry (`carry_rate_limit`, see the fleet rate-limit layer above), so a pane's card can never contradict what its own feed stopped asserting. Per-field arbitration only arises when a second source for a field exists — today statusline is the sole writer.)
- **Claude Code owns its scrollback — so it gets the real mouse** — *verified live (2026-07-12, byte captures of a real session)*: past the trust prompt, Claude Code's main UI switches to the **alternate screen** (`DECSET 1049`), enables SGR mouse tracking (`1000/1002/1003 + 1006`), and implements its own drag-selection — fed press/drag/release reports it repaints a highlight and **copies via OSC 52** on release. Its transcript scrollback is virtualized inside the app; roster's emulator holds **no history** for the pane, so roster-side selection could never span it. The resolution is passthrough, not reimplementation: left-button press/drag/release over a live SGR-mouse-tracking pane forward to the guest (grab semantics — the drag stays routed to the pressed pane), the wheel forwards as before, and guest OSC 52 clipboard writes are relayed to the hosting terminal each frame — focused pane only, with a capped queue, so a background or looping guest cannot hijack the clipboard or grow host memory (`relay_clipboard_writes`; clipboard *reads* stay unanswered — a guest must not see the host clipboard). Selection in a Claude pane is therefore Claude Code's own — scrolling through its transcript exactly as in a bare terminal. Roster's selection (with drag-at-edge history scrolling) serves panes that never asked for the mouse: shells, plain CLIs, and non-mouse full-screen apps — where an alt-screen guest owns the content, an edge-drag hold explains itself with a toast, and a forwarded wheel drops any live roster selection (the repaint re-keys the rows under it). Smoke coverage: `mouse_native_guest_gets_press_drag_and_release`, `guest_osc52_copy_is_relayed_only_from_the_focused_pane`, `edge_drag_on_a_self_scrolling_pane_explains_itself`, `wheel_forward_to_an_alt_screen_guest_drops_the_selection`.
