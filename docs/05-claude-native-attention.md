...
- **Claude Code owns its scrollback** — *verified live (2026-07-12, byte capture of a real session)*: past the trust prompt, Claude Code's main UI switches to the **alternate screen** (`DECSET 1049`) and enables SGR mouse tracking (`1000/1002/1003 + 1006`). Its transcript scrollback is virtualized inside the app — wheel input is forwarded to it, it scrolls and repaints itself (painting its own "Jump to bottom" pill), and roster's emulator holds **no history** for the pane. Consequences, accepted as a structural limit: drag-at-edge history scrolling works only for primary-screen panes (shells, plain CLIs); in a Claude pane, selection covers the visible screen (an edge-drag hold explains this with a toast instead of failing silently), and a forwarded wheel **drops** any live selection (the guest repaint re-keys every row under the highlight — `wheel_forward_to_a_mouse_reporting_guest_drops_the_selection` in smoke.rs guards this). Cross-scrollback selection of Claude output cannot be provided by the multiplexer at all — the data exists only inside Claude Code (the transcript file is off-limits by the rule above); Claude-native affordances (`ctrl-o` transcript view, `/export`) are the sanctioned path.
