...
The ranked cross-workspace queue in `roster-tui`, verbatim reasons, jump-to-answer. This is a UI-taste milestone; it needs eyes on real fleets. The ordering primitive already exists and the sidebar orders by it today, within each workspace: `roster_core::attention` — blocked first (destructive asks above plain ones, then longest wait), then done, then idle, with working at the bottom, because a working agent needs nothing from the human. The inbox reuses the same primitive across workspaces: one triage judgment, owned in one place, so the sidebar and the inbox can never disagree about who needs you first.
